...
function bpageUpdateInfo()
{
    ...
    if(nameFlag == false && emailFlag == false && usernameFlag == false && phoneFlag == false)
    {
        ...
        xhttp.open("POST", "../../admin/controller/updatebpagecheck.php?id="+id, true);
        xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhttp.send("name="+name+"&email="+email+"&phone="+phoneEncode+"&username="+username+"&status="+status);
    }
    //console.log(name+" "+email+" "+phone+" "+dob);
    return false;
}
